...
The app SHALL, when the user taps "Restore wallet" with a valid, complete phrase, import the phrase through WDK restore APIs, unlock the runtime, and navigate to the home screen such that the sign-in/restore stack is not reachable via back navigation.

#### Scenario: Restoring a valid phrase enters the wallet

- **WHEN** the user taps "Restore wallet" with a complete, WDK-valid 12-word phrase entered
...
When restore fails because a wallet already exists, the app SHALL offer a replace flow that deletes the existing wallet before importing a new phrase, with clear destructive warnings.

#### Scenario: Replace with new phrase

- **WHEN** the user chooses replace with new phrase on the restore screen
- **THEN** the app guides through wallet deletion and returns to restore with an empty phrase grid
